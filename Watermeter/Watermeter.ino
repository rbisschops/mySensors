...
* Copyright (C) 2015 Ralph Bisschops
*
...
void setup()
{
	...
	int low = readEeprom(2); //gw.loadState (2);
	if (high > 0 || low > 0) {
		highThreshold = high;
		lowThreshold = low;
	...
	if (SLEEP_MODE || (currentTime - lastSend > sendFrequency)) {
		...
		if ((pulseCount != oldPulseCount) || (!SLEEP_MODE)) {
			...
			if (debugLevel < 2) {
				...
				gw.send(lastCounterMsg.set(pulseCount));
			}
			...
			if ((volume != oldvolume) || (!SLEEP_MODE)) {
				...
				if (debugLevel < 2) {
					...
					gw.send(volumeMsg.set(volume, 3));
				}
			}
		}
	}
	if (SLEEP_MODE) {
		gw.sleep(sendFrequency);
	}
}
...
void incomingMessage(const MyMessage &message)
{
	...
	debugMessage("Pos 0: ", String(gw.loadState(0)));
	debugMessage("Pos 1: ", String(gw.loadState(1)));
	debugMessage("Pos 2: ", String(gw.loadState(2)));
	debugMessage("Pos 3: ", String(gw.loadState(3)));
	debugMessage("Pos 4: ", String(gw.loadState(4)));
}
...
void checkThreshold() {
	...
	if (debugLevel > 0) {
		...
		if (debugLevel == 2) {
			gw.send(analogValue.set(sensorValue));
		}
